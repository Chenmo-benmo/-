...
public:
    std::string                 m_CSaveJpgPath;                             //保存图片的路径
    XYCoord                     FindCoord(std::string path, float x, float y);  //返回坐标
    std::vector<PixelsCoord>    m_CRoi;
    bool                        m_CIsTime = true;

    afx_msg void OnBnClickedxunhuan();
    afx_msg void OnBnClickedZero();
    afx_msg void OnBnClickedone();
    afx_msg void OnBnClickedMovebtn();
    afx_msg void OnBnClickedShow();
    afx_msg void OnBnClickedFindhsvbtn();
    afx_msg void OnTvnSelchangedTree1(NMHDR* pNMHDR, LRESULT* pResult);
    afx_msg void OnBnClickedTreebtn();
    afx_msg void OnBnClickedshaixuanbtn();
};
